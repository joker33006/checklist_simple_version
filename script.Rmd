...
getwd()
write.csv(miss_value,"/result/missing_value.xlsx")
writexl::write_xlsx(tb,"/result/checklist_table.xlsx")
write.csv(iucn,"/result/iucn_count.csv")

```

本研究共調查到`r fam_number`科`r genus_number`屬`r sp_number`種。其中，`r as.character(source[1,1])`種為`r as.character(source[1,2])`種, `r as.character(source[2,1])`種為`r as.character(source[2,2])`種。  
